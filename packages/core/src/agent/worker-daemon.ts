...
export async function startWorkerDaemon(options: WorkerDaemonOptions): Promise<void> {
  const { root, agent, agents, apiConfig, onLog } = options;
  ...
  // --- Register ---
  log(`Registering worker "${workerName}" with agent "${agent}"...`);
  const registration = await registerWorker(apiConfig, workerName, agent, {
    hostname: hostname(),
  ...
  const executeJob = async (job: WorkerJobAssignment) => {
    log(`Received job ${job.job_id} (${job.entity_type})`);
    ...
    try {
      ...
      await workerJobCompleted(apiConfig, job.job_id, exitCode);
    } catch (err) {
      log(`Job execution error: ${(err as Error).message}`);
      try {
        await workerJobCompleted(apiConfig, job.job_id, 1);
      } catch {
        // best effort
      }
    } finally {
